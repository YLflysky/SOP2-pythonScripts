...
if __name__ == '__main__':
    ...
    if not args.mark:
        pytest.main(
            ['-qvs', '-m' './test', '--alluredir=/data/allure-results'])
    else:
        if len(args.mark) > 1:
            pytest.main(
                ['-qvs', '-m' './test', '--alluredir=/data/allure-results', '-m', "{}".format(' or '.join(args.mark))])
        elif len(args.mark) == 1:
            pytest.main(
                ['-qvs', '-m' './test', '--alluredir=/data/allure-results', '-m', "{}".format(args.mark[0])])
        else:
            print('参数错误')
            sys.exit(-1)
