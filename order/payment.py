...
class Payment(Base):
    ...
    def agreement_qr_code(self,aid,channel,origin,service,sp,vin,way):
        '''
        支付底层获取免密签约二维码接口
        :param aid:用户id
        ...
        data = {'aid':aid,'payChannel':channel,'origin':origin,'serviceId':service,'spId':sp,'vin':vin,'payWay':way}
        c,b = self.do_post(url,data)
        self.assert_msg(c,b)
        return b
