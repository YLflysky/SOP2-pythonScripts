...
class Team(MABase):
    ...
    def exchange_position(self,vin,aid,group_id,longitude,latitude,dir,ver):
        '''
        车机端调用此接口和微信小程序同步更新位置
        '''
        url = self.url + '/api/v1/exchangePosition'
        data = {'vin':vin,'accountId':aid,'groupId':group_id,'longitude':longitude,'latitude':latitude,'dir':dir,'ver':ver}
        c,b = self.do_post(url,data)
        self.assert_bm_msg(c,b)
